...
export default function ContentModule(moduleOpts) {
  ...
  const contentDir = userOptions.contentDir
    ? userOptions.contentDir
    : join(this.options.srcDir, CONTENT_DIR)

  const contentDirWebpackAlias = contentDir
  const port =
  ...
  this.nuxt.hook('build:before', async builder => {
    ...
    await buildContent(this, BUILD_DIR, isStatic, routesOptions)
  })

  this.nuxt.hook('generate:distRemoved', async generator => {
    await buildContent(this, BUILD_DIR, true, routesOptions)

    const apiOptions = getAPIOptions(userOptions.api, true)
