...
const generateAssets = async (nuxtOpts, assetMap) => {
  const distPath = resolve(nuxtOpts.buildDir, 'dist')

  assetMap.forEach(async (page, buildPath) => {
    const assetObj = asset(page)
    const filePath = join(distPath, buildPath)
    try {
      await writeFile(filePath, assetObj.source())
    } catch (e) {
    }
  })
}
 ...
 */
const buildContent = async (nuxt, buildDir, isStatic, options) => {
  const { contentDir, content, parsers, isDev, noBuild } = options
  ...
  if (noBuild) {
    await generateAssets(nuxt.options, assetMap)
  } else {
